...
final class Simply_Schedule_Appointments {
	...
	public static function _uninstall(){
		...
		if ( ! empty( $settings->developer->remove_data_on_uninstall) ) {
			...
			$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE 'ssa\_%' OR option_name LIKE 'wp_ssa\_%'" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
		
			// Truncate all ssa database tables.
			$wpdb->query("DROP TABLE wp_ssa_appointments");
			$wpdb->query("DROP TABLE wp_ssa_appointment_meta");
			$wpdb->query("DROP TABLE wp_ssa_appointment_types");
			$wpdb->query("DROP TABLE wp_ssa_availability");
			$wpdb->query("DROP TABLE wp_ssa_availability_external");
			$wpdb->query("DROP TABLE wp_ssa_async_actions");
			$wpdb->query("DROP TABLE wp_ssa_payments");
			$wpdb->query("DROP TABLE wp_ssa_staff");
			$wpdb->query("DROP TABLE wp_ssa_staff_appointments");
			$wpdb->query("DROP TABLE wp_ssa_staff_appointment_types");
			$wpdb->query("DROP TABLE wp_ssa_appointment_type_labels");
			$wpdb->query("DROP TABLE wp_ssa_revisions");
			$wpdb->query("DROP TABLE wp_ssa_revision_meta");
		}
	}
