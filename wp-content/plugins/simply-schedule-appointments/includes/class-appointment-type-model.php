...
class SSA_Appointment_Type_Model extends SSA_Db_Model {
	...
	public function hooks() {
		...
		add_action(  'ssa/settings/staff/updated', array( $this, 'recheck_appointment_types_capacity' ), 10, 2 );
	...
	/**
	 * resets capacity limits after staff feature is disabled
	 *
	 */
	public function recheck_appointment_types_capacity( $settings, $old_settings){
		if ( !empty( $old_settings['enabled'] ) && empty( $settings['enabled'] ) ) {
			$appointment_types = $this->get_all_appointment_types();
			foreach ( $appointment_types as $appointment_type ) {
				if ( $appointment_type['capacity'] == 100000 ) {
	...
	public function get_all_appointment_types_booking_app_new() {
		$transient = ssa_cache_get( 'appointment_types_booking_app_new' );
		...
		if( $this->plugin->settings_installed->is_installed( 'staff' ) ) {
			foreach ( $items as $appointment_type_key => $appointment_type ) {
				...
				$items[$appointment_type_key]['staff_objects'] = $associated_staff;
			}
		}

		// Add Resources to Appointment Type
		if( $this->plugin->settings_installed->is_installed( 'resources' ) ) {
			foreach ( $items as $appointment_type_key => $appointment_type ) {
				...
				foreach( $associated_resource_groups as $resource_group ) {
					...
					$items[$appointment_type_key]['resources_objects'][] = $resource_group;
				}
			}
		}

		$transient = ssa_cache_set( 'appointment_types_booking_app_new', $items, '', MONTH_IN_SECONDS );
