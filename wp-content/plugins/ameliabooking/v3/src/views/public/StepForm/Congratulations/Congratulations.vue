      ...
      <div class="am-fs__congrats-info-customer">
        ...
        <div v-if="booked.price > 0 || (booked.price <= 0 && coupon.code)">
          ...
          <span v-else-if="booked">
            {{ ( booked.payments[0].gateway !== 'onSite' ? useFormattedPrice(booked.paymentAmount) : useFormattedPrice(booked.price < 0 ? 0 : booked.price) ) + (booked.payments[0].status !== 'paid' ?  (' - ' + amLabels.on_site) : '') }}
          </span>
          <span v-else></span>
        </div>
