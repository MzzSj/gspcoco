...
function useAvailableCategories (entities) {
  let arr = []
  entities.categories.forEach(category => {
    let serviceIdsInCategory = useAvailableServiceIdsInCategory(category, entities)
    ...
    entities.packages.forEach(pack => {
      serviceIdsInCategory.forEach(service => {
        if (
          ...
          && usePackageAvailabilityByEmployeeAndLocation(entities, pack)
        ) {
