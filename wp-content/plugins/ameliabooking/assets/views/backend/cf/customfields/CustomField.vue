<template>
  <div class="am-custom-field">
    ...
    <el-row :gutter="16">
      ...
      <!-- Services & Required -->
      <el-col :sm="customField.type === 'address' ? (events.length ? 5 : 15) : (events.length ? 7 : 17)" v-if="categories.length">
        <!-- Services -->
        <div>
      ...
      <el-col :sm="customField.type === 'address' ? (categories.length ? 5 : 15) : (categories.length ? 7 : 17)" v-if="events.length">
        <div>
      ...
      <el-col :lg="5" v-if="customField.type === 'address'">
        <!-- Google Location -->
        <div class="am-customize-required">
          <p class="am-label" style="visibility: hidden;">{{ $root.labels.address_as_location }}:</p>

          <el-checkbox
              v-model="customField.useAsLocation"
              @change="updateCustomField(false)"
              border
          >
            {{ $root.labels.address_as_location }}
            <el-tooltip placement="top">
              <div slot="content" v-html="$root.labels.address_as_location_tooltip"></div>
    ...
    <el-row v-if="customField.type === 'address' && !$root.settings.general.gMapApiKey">
      <el-alert
        title=""
        type="warning"
        :description="$root.labels.address_warning"
        show-icon
        :closable="false"
      >
      </el-alert>
