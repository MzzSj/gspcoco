  ...
  <div>
    ...
    <div class="am-dialog-footer" v-if="$root.settings.capabilities.canWriteOthers === true && !dialogLoading">
      <div class="am-dialog-footer-actions">
        ...
        <transition name="slide-vertical">
          <div class="am-dialog-confirmation" v-if="showRefundConfirmation && !$root.licence.isBasic && !$root.licence.isLite">
            <h3>{{ $root.labels.confirm_refund_payment }}<span style="font-size:inherit;display:inline" v-if="modalData.recurring && modalData.recurring.length > 0"> {{ $root.labels.refund_rec_payment }}</span></h3>
    ...
    methods: {
      showRefundModal (singlePayment) {
        ...
        this.$http.get(`${this.$root.getAjaxUrl}/payments/transaction/` + singlePayment.id)
          .then(response => {
            this.refundAmount = response.data.data.transactionAmount
      ...
      paymentRefundable (payment) {
        return (!this.$root.licence.isBasic && !this.$root.licence.isLite) && (payment.status === 'partiallyPaid' || payment.status === 'paid') && (payment.transactionId || payment.wcOrderId) && (payment.gateway !== 'mollie' || (payment.transactionId && !payment.transactionId.includes('pl_')))
      },
      ...
      setFinance () {
        ...
        $this.modalData.bookings.forEach(function (bookItem) {
          if ($this.modalData.customerBookingId === bookItem.id) {
            ...
            let paidDeposit = 0
            let paidRemaining = 0
            bookItem.payments.filter(payItem => payItem.status !== 'refunded' && payItem.status !== 'pending').forEach(function (payItem) {
              // if (payItem.id === $this.modalData.paymentId) {
